...
export default function HybridBERTRec() {
  ...
  const fetchRecommendations = () => {
    ...
    console.log("[HybridBERTRec] Fetching recommendations for user:", userId);
    setLoading(true);
    setError(null);
    
    fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/recommend/hybrid-bert?user_id=${encodeURIComponent(userId)}`)
      .then((res) => {
        if (!res.ok) {
      ...
      .then((data) => {
        console.log("[HybridBERTRec] Received recommendations:", data.length, "games");
        setGames(data);
        setLoading(false);
      })
      .catch((error) => {
        console.error("[HybridBERTRec] Error fetching recommendations:", error);
